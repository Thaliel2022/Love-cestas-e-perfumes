...
// ===== INÍCIO DA ALTERAÇÃO: Adicionados novos status para retirada =====
const ORDER_STATUS = {
    PENDING: 'Pendente',
...
// ===== INÍCIO DA ALTERAÇÃO: Novo template de e-mail para retirada =====
const createReadyForPickupEmail = (customerName, orderId, items) => {
    const appUrl = process.env.APP_URL || 'http://localhost:3000';
    const itemsHtml = createItemsListHtml(items, "Itens do seu pedido:");
    ...
    const content = `
        ...
        <p style="color: #E5E7EB; font-family: Arial, sans-serif; font-size: 16px; line-height: 1.6; margin: 0 0 15px;">Olá, ${customerName},</p>
        <p style="color: #E5E7EB; font-family: Arial, sans-serif; font-size: 16px; line-height: 1.6; margin: 0 0 15px;">Ótima notícia! Seu pedido #${orderId} já pode ser retirado em nossa loja.</p>
        <div style="border: 1px solid #4B5563; padding: 20px; text-align: left; margin: 20px 0; border-radius: 5px;">
            <h3 style="margin: 0 0 10px 0; color: #D4AF37; font-family: Arial, sans-serif;">Informações para Retirada</h3>
            <p style="margin: 5px 0; color: #E5E7EB; font-family: Arial, sans-serif;"><strong>Endereço:</strong> R. Leopoldo Pereira Lima, 378 – Mangabeira VIII, João Pessoa – PB</p>
            <p style="margin: 5px 0; color: #E5E7EB; font-family: Arial, sans-serif;"><strong>Horário:</strong> Segunda a sábado, das 9h às 11h30 e das 15h às 17h30.</p>
            <p style="margin: 15px 0 0 0; color: #E5E7EB; font-size: 14px; font-family: Arial, sans-serif;"><strong>Importante:</strong> Para retirar, é indispensável apresentar um documento com foto (RG ou CNH) e o número do pedido.</p>
        </div>
        ${itemsHtml}
        <table width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation"><tr><td align="center" style="padding: 20px 0;"><a href="${appUrl}/#account/orders" target="_blank" style="display: inline-block; padding: 12px 25px; background-color: #D4AF37; color: #111827; text-decoration: none; border-radius: 5px; font-weight: bold; font-family: Arial, sans-serif;">Ver Detalhes do Pedido</a></td></tr></table>
    `;
...
// --- ROTAS DE PEDIDOS ---
// ===== INÍCIO DA ALTERAÇÃO: Rota de consulta de pedidos atualizada =====
...
    try {
        // ===== INÍCIO DA ALTERAÇÃO: Adiciona authorized_pickup_by na consulta =====
...
// ===== INÍCIO DA ALTERAÇÃO: Rota de criação de pedido atualizada para retirada na loja =====
app.post('/api/orders', verifyToken, async (req, res) => {
    const { items, total, shippingAddress, paymentMethod, shipping_method, shipping_cost, coupon_code, discount_amount, authorized_pickup_by } = req.body;
...
        }
       
        const orderSql = "INSERT INTO orders (user_id, total, status, shipping_address, payment_method, shipping_method, shipping_cost, coupon_code, discount_amount, authorized_pickup_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        const orderParams = [
            req.user.id, 
            total, 
            ...
            coupon_code || null, 
            discount_amount || 0,
            authorized_pickup_by ? JSON.stringify(authorized_pickup_by) : null
        ];
...
// ===== INÍCIO DA ALTERAÇÃO: Rota de atualização de pedido atualizada =====
...
        }
        const { status: currentStatus, payment_gateway_id, payment_status: currentPaymentStatus, shipping_method } = currentOrderResult[0];
        const isPickup = shipping_method === 'Retirar na loja';
        const STATUS_PROGRESSION = isPickup ? PICKUP_PROGRESSION : DELIVERY_PROGRESSION;
...
                    emailSubject = `Seu Pedido #${id} foi enviado!`;
                    ...
                    emailHtml = createReadyForPickupEmail(customerName, id, parsedItems);
