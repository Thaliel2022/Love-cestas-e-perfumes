...
// --- ROTAS DE GERENCIAMENTO DE TEMAS (Admin & Público) ---

// (Público) Retorna o tema ativo atual
app.get('/api/themes/current', async (req, res) => {
    try {
        const connection = await db.getConnection();
        
        // 1. Prioridade Máxima: Tema Manual
        const [manualTheme] = await connection.query("SELECT * FROM site_themes WHERE is_active_manual = 1 LIMIT 1");
        if (manualTheme.length > 0) {
            connection.release();
            return res.json(manualTheme[0]);
        }

        // 2. Prioridade Média: Agendamento por Data
        const today = new Date().toISOString().split('T')[0];
        const [scheduledTheme] = await connection.query(
            "SELECT * FROM site_themes WHERE start_date <= ? AND end_date >= ? LIMIT 1",
            [today, today]
        );
        ...
        const [defaultTheme] = await connection.query("SELECT * FROM site_themes WHERE name = 'Padrão' LIMIT 1");
        connection.release();
        
        if (defaultTheme.length > 0) {
            return res.json(defaultTheme[0]);
        }
        ...
        console.error("Erro ao buscar tema atual:", err);
...
app.get('/api/themes', verifyToken, verifyAdmin, async (req, res) => {
    try {
        const [themes] = await db.query("SELECT * FROM site_themes ORDER BY id ASC");
        res.json(themes);
    } catch (err) {
...
// (Admin) Criar ou Editar Tema
app.post('/api/themes', verifyToken, verifyAdmin, async (req, res) => {
    const { id, name, colors, typography, decoration, start_date, end_date, is_active_manual } = req.body;
    const colorsString = typeof colors === 'object' ? JSON.stringify(colors) : colors;

    try {
        if (is_active_manual) {
            await db.query("UPDATE site_themes SET is_active_manual = 0");
        }

        if (id) {
            // Bloqueio de edição do Padrão
            const [existing] = await db.query("SELECT name FROM site_themes WHERE id = ?", [id]);
            if (existing.length > 0 && existing[0].name === 'Padrão') {
                // Se for Padrão, só permite alterar 'is_active_manual' para desligar, nada mais.
                // Mas para segurança, vamos bloquear edição completa por aqui.
                return res.status(403).json({ message: "O tema Padrão é protegido." });
            }

            await db.query(
                "UPDATE site_themes SET name=?, colors=?, typography=?, decoration=?, start_date=?, end_date=?, is_active_manual=? WHERE id=?",
                [name, colorsString, typography, decoration, start_date || null, end_date || null, is_active_manual ? 1 : 0, id]
            );
            logAdminAction(req.user, 'EDITOU TEMA', `ID: ${id}`);
            res.json({ message: "Tema atualizado!" });
        } else {
            await db.query(
                "INSERT INTO site_themes (name, colors, typography, decoration, start_date, end_date, is_active_manual) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [name, colorsString, typography, decoration, start_date || null, end_date || null, is_active_manual ? 1 : 0]
            );
...
app.delete('/api/themes/:id', verifyToken, verifyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const [theme] = await db.query("SELECT name FROM site_themes WHERE id = ?", [id]);
        if (theme.length > 0 && theme[0].name === 'Padrão') {
            return res.status(403).json({ message: "O tema padrão não pode ser excluído." });
        }
        await db.query("DELETE FROM site_themes WHERE id = ?", [id]);
