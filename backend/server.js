...
const webpush = require('web-push'); // <-- NOVO: Biblioteca para notificações push
...
        'RESEND_API_KEY', 'FROM_EMAIL',
        'VAPID_PUBLIC_KEY', 'VAPID_PRIVATE_KEY', 'VAPID_SUBJECT' // <-- NOVO: Variáveis para Push
    ];
...
// --- NOVO: Função para enviar notificações ---
const sendPushNotification = async (userId, payload) => {
    try {
        const [subscriptions] = await db.query("SELECT subscription_object FROM push_subscriptions WHERE user_id = ?", [userId]);
        if (subscriptions.length === 0) {
            console.log(`[Push] Nenhum registro de notificação encontrado para o usuário #${userId}.`);
            return;
        }

        const notificationPromises = subscriptions.map(sub => {
            const subscription = JSON.parse(sub.subscription_object);
            return webpush.sendNotification(subscription, JSON.stringify(payload))
                .catch(err => {
                    // Se a inscrição expirou ou é inválida, o push service retorna um erro (ex: 410 Gone)
                    if (err.statusCode === 410 || err.statusCode === 404) {
                        console.log(`[Push] Inscrição para o usuário #${userId} expirou. Removendo do DB.`);
                        return db.query("DELETE FROM push_subscriptions WHERE subscription_object = ?", [sub.subscription_object]);
                    } else {
                        console.error(`[Push] Erro ao enviar notificação para user #${userId}:`, err);
                    }
                });
        });

        await Promise.all(notificationPromises);
        console.log(`[Push] Notificações para o usuário #${userId} enviadas com sucesso.`);
    } catch (error) {
        console.error(`[Push] Erro geral ao processar notificações para o usuário #${userId}:`, error);
    }
};
...
// --- NOVO: ROTAS PARA NOTIFICAÇÕES PUSH ---
...
app.post('/api/subscribe', verifyToken, async (req, res) => {
    ...
    if (!subscription || !subscription.endpoint) {
        return res.status(400).json({ message: 'Objeto de inscrição inválido.' });
    }

    try {
        const subscriptionJson = JSON.stringify(subscription);
        // Usar INSERT IGNORE para evitar erros de duplicidade se o usuário se inscrever novamente com o mesmo dispositivo
        await db.query("INSERT IGNORE INTO push_subscriptions (user_id, subscription_object) VALUES (?, ?)", [userId, subscriptionJson]);
        res.status(201).json({ message: 'Inscrição para notificações realizada com sucesso.' });
    } catch (error) {
        console.error("Erro ao salvar a inscrição de notificação:", error);
        res.status(500).json({ message: 'Falha ao salvar a inscrição.' });
    }
});

app.post('/api/notifications/broadcast', verifyToken, verifyAdmin, async (req, res) => {
    const { title, body, url, image } = req.body;

    if (!title || !body || !url) {
        return res.status(400).json({ message: 'Título, corpo e URL são obrigatórios.' });
    }
    ...
    try {
        ...
        const payload = JSON.stringify({
            title,
            body,
            icon: 'https://res.cloudinary.com/dvflxuxh3/image/upload/v1752292990/uqw1twmffseqafkiet0t.png', // Ícone padrão
            badge: 'https://res.cloudinary.com/dvflxuxh3/image/upload/v1752296170/kk9tlhxb2qyioeoieq6g.png', // Badge padrão
            image, // Imagem opcional para a notificação
            data: { url }
        });
        ...
        res.status(200).json({ message: `Notificações enviadas. Sucesso: ${successCount}. Falhas: ${failureCount}.` });

    } catch (error) {
        console.error("Erro ao enviar notificações em massa:", error);
        res.status(500).json({ message: 'Erro interno ao enviar notificações.' });
    }
});
...
// ATUALIZADO: Rota de atualização de pedido com envio de e-mail e notificação push
...
        ORDER_STATUS.PROCESSING, ORDER_STATUS.SHIPPED, ORDER_STATUS.OUT_FOR_DELIVERY,
        ORDER_STATUS.DELIVERED, ORDER_STATUS.CANCELLED, ORDER_STATUS.REFUNDED
    ];
    
    // NOVO: Status que também disparam notificações push
    const statusesThatTriggerPush = [
        ORDER_STATUS.SHIPPED, ORDER_STATUS.OUT_FOR_DELIVERY, ORDER_STATUS.DELIVERED, ORDER_STATUS.CANCELLED
    ];
...
        if (currentOrderResult.length === 0) throw new Error("Pedido não encontrado.");
        
        const { status: currentStatus, payment_gateway_id, payment_status: currentPaymentStatus, user_id: orderUserId } = currentOrderResult[0];
...
        await connection.commit();
        
        // --- LÓGICA DE ENVIO DE E-MAIL E PUSH ---
        try {
            if (status && (statusesThatTriggerEmail.includes(status) || statusesThatTriggerPush.includes(status))) {
                const [userResult] = await db.query("SELECT email, name FROM users WHERE id = ?", [orderUserId]);
                if (userResult.length > 0) {
                    const { email: customerEmail, name: customerName } = userResult[0];
...
                    if (statusesThatTriggerEmail.includes(status)) {
                        ...
                        if (emailHtml && emailSubject) {
                            await resend.emails.send({ from: FROM_EMAIL, to: customerEmail, subject: emailSubject, html: emailHtml });
                            console.log(`E-mail de atualização de status "${status}" enviado para ${customerEmail} para o pedido #${id}.`);
                        }
                    }

                    // NOVO: Envio de Notificação Push
                    if (statusesThatTriggerPush.includes(status)) {
                        const pushPayload = {
                            title: `Pedido #${id}: ${status}`,
                            body: status === ORDER_STATUS.SHIPPED 
                                ? `Seu pedido foi enviado! Cód. rastreio: ${finalTrackingCode}`
                            ...
                            badge: 'https://res.cloudinary.com/dvflxuxh3/image/upload/v1752296170/kk9tlhxb2qyioeoieq6g.png',
                            data: { url: `${process.env.APP_URL}/#account/orders` }
                        };
                        await sendPushNotification(orderUserId, pushPayload);
                    }
                }
            }
        } catch (notificationError) {
            console.error(`FALHA AO ENVIAR E-MAIL/PUSH para o pedido #${id}:`, notificationError);
        }
