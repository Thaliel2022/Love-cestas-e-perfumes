...
const ThemeEffects = memo(({ type }) => {
    ...
    switch (type) {
        case 'snow': return renderParticles('❄️', 50, 8);
        case 'hearts': return renderParticles('❤️', 30, 6);
        case 'flowers': return renderParticles('🌸', 25, 7);
        case 'confetti': return renderParticles('🎉', 40, 4);
        case 'halloween': return renderParticles('🎃', 20, 6);
        case 'stars': return renderParticles('✨', 35, 5);
        case 'money': return renderParticles('💸', 25, 3);
        case 'easter': return renderParticles('🐰', 20, 6);
        default: return null;
    }
});
// --- CONTEXTOS GLOBAIS ---
...
const ShopProvider = ({ children }) => {
    ...
    // Cores padrão (ORIGINAL DO SITE: PRETO E DOURADO)
    ...
    const determineShippingLocation = useCallback(async () => {
        ...
        if (!locationDetermined && navigator.geolocation) {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => { setIsGeolocating(false); }, { timeout: 10000 }
            );
        }
    ...
    useEffect(() => {
        ...
        if (isAuthenticated) { fetchPersistentCart(); determineShippingLocation(); apiService('/wishlist').then(setWishlist).catch(console.error); } 
        else { setCart([]); setWishlist([]); setAddresses([]); setShippingLocation({ cep: '', city: '', state: '', alias: '' }); setAutoCalculatedShipping(null); setCouponCode(''); setAppliedCoupon(null); setCouponMessage(''); determineShippingLocation(); }
    ...
    useEffect(() => {
        ...
        const debounceTimer = setTimeout(() => {
            if (itemsToCalculate && itemsToCalculate.length > 0 && shippingLocation.cep.replace(/\D/g, '').length === 8) {
                setIsLoadingShipping(true); setShippingError('');
                ...
                calculateShipping();
            } else { setShippingOptions([]); setAutoCalculatedShipping(null); }
        }, 500);
    ...
    const addToCart = useCallback(async (productToAdd, qty = 1, variation = null) => {
        ...
        if ((existing ? existing.qty : 0) + qty > availableStock) throw new Error(`Estoque insuficiente.`);

        setCart(currentCart => {
            let updatedCart;
            if (existing) updatedCart = currentCart.map(item => item.cartItemId === cartItemId ? { ...item, qty: item.qty + qty } : item);
            else updatedCart = [...currentCart, { ...productToAdd, qty, variation, cartItemId }];
    ...
    const addToWishlist = useCallback(async (productToAdd) => {
        ...
        } catch (error) { return { success: false, message: `Erro ao adicionar: ${error.message}` }; }
    ...
    const applyCoupon = useCallback(async (code) => {
        ...
        } catch (error) { removeCoupon(); setCouponMessage(error.message || "Erro no cupom."); }
    }, [removeCoupon]);
    ...
    return (
        <ShopContext.Provider value={{
            ...
            updateDefaultShippingLocation, determineShippingLocation, setPreviewShippingItem, setSelectedShippingName,
            isGeolocating, couponCode, setCouponCode, couponMessage, applyCoupon, appliedCoupon, removeCoupon,
...
const ProductCard = memo(({ product, onNavigate }) => {
    ...
    useEffect(() => { setIsPromoActive(!!product.is_on_sale && product.sale_price > 0); }, [product]);

    const currentPrice = isPromoActive ? product.sale_price : product.price;
    const discountPercent = useMemo(() => {
        if (isPromoActive && product.price > 0) return Math.round(((product.price - product.sale_price) / product.price) * 100);
        return 0;
    ...
    return (
        ...
        >
            <div className="relative h-64 bg-white overflow-hidden">
                ...
                <button onClick={toggleWishlist} className={`absolute top-2 right-2 p-1.5 rounded-full z-10 transition-colors ${isWishlisted ? 'text-[var(--theme-primary)] bg-black/10' : 'text-gray-400 hover:text-[var(--theme-primary)]'}`}>
                    <HeartIcon className="h-5 w-5" filled={isWishlisted} />
                </button>
                <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                    {product.stock <= 0 ? <div className="bg-gray-800 text-white text-[10px] font-bold px-2 py-1 rounded">ESGOTADO</div> : isPromoActive ? <div className="bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm">PROMO -{discountPercent}%</div> : isNew ? <div className="bg-blue-500 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm">NOVO</div> : null}
                </div>
            </div>

            <div className="p-4 flex flex-col flex-grow">
                <p className="text-xs font-bold tracking-wider opacity-60 mb-1">{product.brand.toUpperCase()}</p>
                    ...
                    {isPromoActive ? (
                         <div className="flex flex-col">
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                {timeLeft && timeLeft !== 'Expirada' && <span className="text-[10px] text-red-500 font-bold bg-red-100 px-1 rounded">{timeLeft}</span>}
                            </div>
                        </div>
                    ) : ( <p className="text-lg font-bold">R$ {Number(product.price).toFixed(2)}</p> )}
                    
                    <div className="mt-3 flex gap-2">
                        <button onClick={(e) => {e.stopPropagation(); onNavigate(`product/${product.id}`)}} className="flex-1 py-2 rounded text-sm font-bold text-black transition hover:opacity-90 flex items-center justify-center gap-1" style={{ backgroundColor: 'var(--theme-primary)' }}>
                            <EyeIcon className="h-4 w-4"/> Ver
                        </button>
                        <button onClick={handleAddToCart} disabled={isAddingToCart} className="p-2 border border-white/20 rounded hover:bg-white/10 transition disabled:opacity-50">
...
const Header = memo(({ onNavigate }) => {
    ...
    useEffect(() => {
        ...
        const controlNavbar = () => {
            ...
            if (window.innerWidth < 768) {
                if (window.scrollY > lastScrollY + 5) setIsBottomNavVisible(false);
                else if (window.scrollY < lastScrollY - 5) setIsBottomNavVisible(true);
            } else setIsBottomNavVisible(true);
    ...
    const fetchAndBuildMenu = useCallback(() => {
        apiService('/collections').then(data => {
            const groupedMenu = data.reduce((acc, c) => {
                if (!acc[c.menu_section]) acc[c.menu_section] = [];
                acc[c.menu_section].push({ name: c.name, filter: c.filter });
                return acc;
            }, {});
            const order = ['Perfumaria', 'Roupas', 'Conjuntos', 'Moda Íntima', 'Calçados', 'Acessórios'];
            setDynamicMenuItems(order.filter(s => groupedMenu[s]).map(s => ({ name: s, sub: groupedMenu[s] })));
    ...
    const prevTotal = useRef(totalCartItems);
    useEffect(() => { if (totalCartItems > prevTotal.current) cartAnimationControls.start({ scale: [1, 1.25, 1], transition: { duration: 0.3 } }); prevTotal.current = totalCartItems; }, [totalCartItems, cartAnimationControls]);

    const handleSearchSubmit = (e) => { e.preventDefault(); if (searchTerm.trim()) { onNavigate(`products?search=${encodeURIComponent(searchTerm.trim())}`); setSearchTerm(''); setIsSearchFocused(false); setIsMobileMenuOpen(false); } };
    const handleSuggestionClick = (id) => { onNavigate(`product/${id}`); setSearchTerm(''); setIsSearchFocused(false); setIsMobileMenuOpen(false); };
    const handleManualCepSubmit = async (e) => { e.preventDefault(); setCepError(''); const c = manualCep.replace(/\D/g, ''); if (c.length !== 8) return setCepError("CEP inválido"); try { const res = await fetch(`https://viacep.com.br/ws/${c}/json/`); const data = await res.json(); if (data.erro) setCepError("Não encontrado"); else { setShippingLocation({ cep: manualCep, city: data.localidade, state: data.uf, alias: `CEP ${manualCep}` }); setIsAddressModalOpen(false); setManualCep(''); } } catch { setCepError("Erro ao buscar"); } };

    let addressDisplay = 'Selecione endereço';
    if (shippingLocation?.cep?.replace(/\D/g, '').length === 8) addressDisplay = `Enviar para ${shippingLocation.alias || 'Local'} - ${shippingLocation.cep}`;

    const BottomNavBar = () => (
        <motion.div className="fixed bottom-0 left-0 right-0 h-16 flex justify-around items-center z-40 md:hidden border-t shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]" initial={false} animate={isBottomNavVisible ? { y: 0 } : { y: "100%" }} style={{ backgroundColor: 'var(--theme-secondary)', borderColor: 'rgba(255,255,255,0.1)' }}>
            <button onClick={() => { onNavigate('home'); setIsMobileMenuOpen(false); }} className="flex flex-col items-center w-1/5 text-[var(--theme-text)] opacity-80 hover:opacity-100 hover:text-[var(--theme-primary)]"><HomeIcon className="h-6 w-6 mb-1"/><span className="text-xs">Início</span></button>
            <button onClick={() => { isAuthenticated ? onNavigate('account') : onNavigate('login'); setIsMobileMenuOpen(false); }} className="flex flex-col items-center w-1/5 text-[var(--theme-text)] opacity-80 hover:opacity-100 hover:text-[var(--theme-primary)]"><UserIcon className="h-6 w-6 mb-1"/><span className="text-xs">Conta</span></button>
            <button onClick={() => { onNavigate('wishlist'); setIsMobileMenuOpen(false); }} className="relative flex flex-col items-center w-1/5 text-[var(--theme-text)] opacity-80 hover:opacity-100 hover:text-[var(--theme-primary)]"><HeartIcon className="h-6 w-6 mb-1"/><span className="text-xs">Lista</span>{wishlist.length > 0 && <span className="absolute top-0 right-[20%] bg-[var(--theme-primary)] text-black text-[10px] rounded-full h-4 w-4 flex items-center justify-center font-bold">{wishlist.length}</span>}</button>
            <button onClick={() => { onNavigate('cart'); setIsMobileMenuOpen(false); }} className="relative flex flex-col items-center w-1/5 text-[var(--theme-text)] opacity-80 hover:opacity-100 hover:text-[var(--theme-primary)]"><motion.div animate={cartAnimationControls}><CartIcon className="h-6 w-6 mb-1"/></motion.div><span className="text-xs">Carrinho</span>{totalCartItems > 0 && <span className="absolute top-0 right-[20%] bg-[var(--theme-primary)] text-black text-[10px] rounded-full h-4 w-4 flex items-center justify-center font-bold">{totalCartItems}</span>}</button>
            <button onClick={() => setIsMobileMenuOpen(true)} className="flex flex-col items-center w-1/5 text-[var(--theme-text)] opacity-80 hover:opacity-100 hover:text-[var(--theme-primary)]"><MenuIcon className="h-6 w-6 mb-1"/><span className="text-xs">Menu</span></button>
        </motion.div>
    );
            ...
            {isAddressModalOpen && (
                <Modal isOpen={true} onClose={() => setIsAddressModalOpen(false)} title="Endereço de Entrega" size="md">
                    <div className="space-y-4">
                        {isAuthenticated && addresses.map(addr => <div key={addr.id} onClick={() => {setShippingLocation({ cep: addr.cep, city: addr.localidade, state: addr.uf, alias: addr.alias }); setIsAddressModalOpen(false);}} className="p-4 border rounded cursor-pointer hover:border-[var(--theme-primary)]"><p className="font-bold">{addr.alias}</p><p className="text-sm">{addr.logradouro}, {addr.numero} - {addr.bairro}</p></div>)}
                        <div className="pt-4 border-t"><form onSubmit={handleManualCepSubmit} className="flex gap-2"><input type="text" value={manualCep} onChange={(e) => setManualCep(e.target.value)} placeholder="00000-000" className="w-full p-2 border rounded text-black" /><button type="submit" className="bg-[var(--theme-primary)] text-black font-bold px-4 rounded">OK</button></form>{cepError && <p className="text-red-500 text-xs mt-1">{cepError}</p>}</div>
                    </div>
                </Modal>
            )}
        </AnimatePresence>
                ...
                <div className="flex justify-between items-center py-3">
                    ...
                    <div className="flex-1 max-w-2xl mx-8">
                         <form onSubmit={handleSearchSubmit} className="relative">
                           <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onFocus={() => setIsSearchFocused(true)} onBlur={() => setTimeout(() => setIsSearchFocused(false), 200)} placeholder="O que você procura?" className="w-full px-5 py-2 rounded-full focus:outline-none focus:ring-2 text-black bg-white/90" style={{ focusRing: 'var(--theme-primary)' }}/>
                           ...
                           <AnimatePresence>{isSearchFocused && searchTerm.length > 0 && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-xl z-50 overflow-hidden text-black"><div className="max-h-96 overflow-y-auto">{searchSuggestions.length > 0 ? searchSuggestions.map(p => (<div key={p.id} onClick={() => handleSuggestionClick(p.id)} className="flex items-center p-3 hover:bg-gray-100 cursor-pointer border-b last:border-b-0"><img src={getFirstImage(p.images)} alt={p.name} className="w-16 h-16 object-contain mr-4 rounded-md bg-white p-1 border" /><div className="flex-grow"><p className="font-semibold">{p.name}</p><p className="text-gray-700 font-bold">R$ {Number(p.sale_price || p.price).toFixed(2)}</p></div></div>)) : <p className="p-4 text-center text-sm text-gray-500">Nenhum produto encontrado.</p>}</div></motion.div>)}</AnimatePresence>
                        </form>
                    </div>
                            ...
                            {isAuthenticated ? (
                                <div className="relative group">
                                   ...
                                   <div className="absolute top-full right-0 w-48 bg-white rounded-md shadow-lg py-1 z-20 invisible group-hover:visible border border-gray-200 text-gray-800"> 
                                        ...
                                        {user.role === 'admin' && (
                                            <a href="#admin" onClick={(e) => { e.preventDefault(); onNavigate('admin/dashboard');}} className="block px-4 py-2 text-sm font-bold text-amber-600 hover:bg-gray-100 border-t border-b border-gray-100">Painel Admin</a>
                                        )} 
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="relative">
                    <div className="overflow-hidden" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
                        <motion.div className="flex -mx-2" animate={{ x: `-${currentIndex * (100 / itemsPerPage)}%` }} transition={{ type: 'spring', stiffness: 350, damping: 40 }}>
                            {categories.map(cat => (
                    ...
                    {canGoPrev && <button onClick={goPrev} className="absolute top-1/2 left-0 transform -translate-y-1/2 -translate-x-2 md:-translate-x-4 bg-white text-black p-2 rounded-full shadow-lg z-10 hidden md:flex hover:scale-110 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></button>}
                    {canGoNext && <button onClick={goNext} className="absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-2 md:translate-x-4 bg-white text-black p-2 rounded-full shadow-lg z-10 hidden md:flex hover:scale-110 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></button>}
                </div>
            </div>
        </section>
    );
        ...
        apiService('/themes')
            .then(data => {
                const sorted = data.sort((a, b) => {
                    // Manual active first, then date active, then ID
                    const aManual = !!a.is_active_manual;
                    const bManual = !!b.is_active_manual;
   ...
   const menuItems = [
    ...
    return (
        <div className="h-screen flex overflow-hidden bg-gray-100 text-gray-800">
            {/* Sidebar - Fundo Escuro Restaurado */}
