...
const CheckoutPage = ({ onNavigate }) => {
        ...
        fetchAddresses().then(userAddresses => {
            ...
            if (shippingLocation && shippingLocation.cep) {
                const matchingSavedAddress = userAddresses.find(addr =>
                    ...
                    (shippingLocation.alias && !shippingLocation.alias.startsWith('CEP ') && shippingLocation.alias !== 'Localização Atual'
                        ? addr.alias === shippingLocation.alias
                        : true)
                );
    ...
    // --- CORREÇÃO: inicializa nome/CPF apenas quando muda a opção de retirada ---
    useEffect(() => {
        if (!autoCalculatedShipping?.isPickup) return; // só aplica se for retirada
    ...
    }, [autoCalculatedShipping?.isPickup, isSomeoneElsePickingUp, user]);
    // ✅ Esse efeito agora executa apenas quando muda a entrega/retirada ou checkbox,
    // sem interferir na digitação contínua dos campos.
    ...
    const subtotal = useMemo(() =>
        cart.reduce((sum, item) =>
            (item.is_on_sale && item.sale_price ? item.sale_price : item.price) * item.qty + sum, 0),
        [cart]
    );
    ...
    const discount = useMemo(() => {
        ...
        let val = 0;
        if (appliedCoupon.type === 'percentage')
            val = subtotal * (parseFloat(appliedCoupon.value) / 100);
        else if (appliedCoupon.type === 'fixed')
            val = parseFloat(appliedCoupon.value);
        else if (appliedCoupon.type === 'free_shipping')
            val = shippingCost;
        return (appliedCoupon.type !== 'free_shipping' && val > (subtotal + shippingCost))
    ...
    const handlePlaceOrderAndPay = async () => {
        ...
        if ((!displayAddress && !isPickup) || !paymentMethod || !autoCalculatedShipping) {
            notification.show("Selecione a forma de entrega e o endereço (se aplicável).", 'error');
            return;
        }
        ...
        try {
            const finalShippingAddress =
                (isPickup || !displayAddress || !displayAddress.id) ? null : displayAddress;
    ...
    // --- Handlers dos inputs ---
    const handlePickupNameChange = (e) => setPickupPersonName(e.target.value);
    const handlePickupCpfChange = (e) => setPickupPersonCpf(maskCPF(e.target.value));
        ...
        <>
            ...
            <div className="bg-black text-white min-h-screen py-8 sm:py-12">
                <div className="container mx-auto px-4">
                    ...
                    {/* (não precisa alterar nada após essa parte) */}
                </div>
            </div>
        </>
    );
