...
const AdminOrders = () => {
    ...
    return (
        <div>
            {/* Modal de Reembolso (Mantido simples) */}
            <AnimatePresence>
                {isRefundModalOpen && editingOrder && (
                    <Modal isOpen={true} onClose={() => setIsRefundModalOpen(false)} title="Solicitar Reembolso">
                         {/* ... form de reembolso igual ao anterior ... */}
                         <div className="p-4">Formulário de Reembolso aqui (mantido lógica original)</div>
                    </Modal>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {editingOrder && (
                    <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title={`Pedido #${editingOrder.id}`} size="3xl">
                        <div className="bg-gray-50/50 -m-6 p-6"> {/* Container com fundo suave */}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 space-y-6">
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                        ...
                                        {/* Resumo Financeiro no Rodapé da Tabela */}
                                    ...
                                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                        <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                            <EditIcon className="h-5 w-5 text-indigo-600"/> Atualizar Pedido
                                        </h4>
                                        <form onSubmit={handleSaveOrder} className="space-y-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                {editingOrder.shipping_method !== 'Retirar na loja' && (
                                                    <div>
                                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Rastreio / Link</label>
                                                        <input 
                                                            type="text" 
                                                            name="tracking_code" 
                                                            value={editFormData.tracking_code} 
                                                            onChange={(e) => setEditFormData({...editFormData, tracking_code: e.target.value})} 
                                                            placeholder="Cód. Correios ou Link Uber" 
                                                            className="w-full p-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"
                                                        />
                                                    </div>
                                                )}
                                            ...
                                            <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                                                <button type="button" onClick={handleManualWhatsAppNotification} className="text-green-600 hover:text-green-700 text-sm font-bold flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-green-50 transition-colors">
                                                    <WhatsappIcon className="h-5 w-5"/> Notificar Cliente
                                                </button>
                                                <div className="flex gap-3">
                                                    <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 text-sm">Cancelar</button>
                                                    <button type="submit" className="px-6 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 shadow-sm text-sm">Salvar Alterações</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <DetailCard title="Cliente" icon={UserIcon}>
                                        <div className="flex items-center gap-3 mb-3">
                                            ...
                                            <div>
                                                <p className="font-bold text-gray-900 leading-tight">{editingOrder.user_name}</p>
                                    ...
                                    <DetailCard title="Pagamento" icon={CreditCardIcon}>
                                        <div className="flex flex-col gap-2">
                                            {(() => {
                                                const method = editingOrder.payment_method;
            ...
            {/* ... Resto da listagem de pedidos (Tabela/Cards da Home do Admin) ... */}
            ...
            <div className="bg-white p-4 rounded-lg shadow-md mb-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <input type="date" name="startDate" value={filters.startDate} onChange={e => setFilters({...filters, startDate: e.target.value})} className="p-2 border rounded-md" title="Data de Início"/>
                    <input type="date" name="endDate" value={filters.endDate} onChange={e => setFilters({...filters, endDate: e.target.value})} className="p-2 border rounded-md" title="Data Final"/>
                    <input type="text" name="customerName" placeholder="Nome do Cliente" value={filters.customerName} onChange={e => setFilters({...filters, customerName: e.target.value})} className="p-2 border rounded-md md:col-span-2"/>
                    <select name="status" value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})} className="p-2 border rounded-md bg-white">
