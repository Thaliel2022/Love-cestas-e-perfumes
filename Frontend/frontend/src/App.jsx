...
const CheckoutPage = ({ onNavigate }) => {
    ...
    // Estados que armazenam o valor FINAL (atualizado no onBlur) - Mantendo a lógica que funcionou para Nome
    ...
    // --- Cálculos de Valores (REVISADOS para garantir números) ---
    ...
    const discount = useMemo(() => {
        ...
        const currentSubtotal = Number(subtotal) || 0; // Garante que subtotal é número aqui
        const currentShippingCost = Number(shippingCost) || 0; // Garante que shippingCost é número aqui
        ...
        // Garante que o desconto não seja maior que o valor total (exceto frete grátis) e não seja negativo
        return Math.max(0, (appliedCoupon.type !== 'free_shipping' && val > currentTotalBeforeDiscount) ? currentTotalBeforeDiscount : val);
    }, [appliedCoupon, subtotal, shippingCost]);


    const total = useMemo(() => {
         // Garante que todos os componentes sejam números antes da soma/subtração
        const finalTotal = (Number(subtotal) || 0) - (Number(discount) || 0) + (Number(shippingCost) || 0);
        return Math.max(0, finalTotal); // Evita total negativo
    }, [subtotal, discount, shippingCost]);
    // --- FIM DA REVISÃO dos Cálculos ---
    ...
    // CORREÇÃO: Função getDeliveryDateText revisada
    const getDeliveryDateText = (deliveryTime) => {
        ...
        // Adiciona um dia de margem para postagem/processamento inicial
        date.setDate(date.getDate() + 1);
        if (date.getDay() === 0) date.setDate(date.getDate() + 1); // Pula domingo
        else if (date.getDay() === 6) date.setDate(date.getDate() + 2); // Pula sábado e domingo

        while (addedBusinessDays < timeInDays) {
            date.setDate(date.getDate() + 1);
            if (date.getDay() !== 0 && date.getDay() !== 6) { // Conta apenas dias úteis
                addedBusinessDays++;
            }
        }
                                    ...
                                    <div className="mt-5 space-y-3">
                                        ...
                                        {isSomeoneElsePickingUp && (
                                            <div className="space-y-2 overflow-hidden bg-gray-800 p-3 rounded-md border border-gray-700">
                                                ...
                                                <input
                                                    type="text"
                                                    value={pickupPersonCpf} // Controlado
                                                    onChange={handlePickupCpfChange} // Atualiza estado com máscara
