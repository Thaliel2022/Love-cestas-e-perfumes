...
const ProductCard = memo(({ product, onNavigate }) => {
    ...
    const isPromo = product.promo_price && parseFloat(product.promo_price) < parseFloat(product.price);
    // Considera o produto como novo se foi criado nos últimos 15 dias
    const isNew = new Date(product.created_at) > new Date(Date.now() - 15 * 24 * 60 * 60 * 1000);

    let discountPercent = 0;
    if (isPromo) {
        discountPercent = Math.round(((parseFloat(product.price) - parseFloat(product.promo_price)) / parseFloat(product.price)) * 100);
    }
        ...
        >
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-y-2">
                    {isPromo && (
                        <motion.div 
                           ...
                           className="bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg flex items-center gap-1"
                        >
                    ...
                    {!isPromo && isNew && (
                        <motion.div 
                           initial={{opacity: 0, x: -20}} animate={{opacity: 1, x: 0}}
                           className="bg-blue-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg flex items-center gap-1"
                        >
            ...
            <div className="p-5 flex-grow flex flex-col">
                ...
                <div className="flex items-center mt-2">
                    ...
                    {product.review_count > 0 && <span className="text-xs text-gray-500 ml-2">({product.review_count})</span>}
                </div>
