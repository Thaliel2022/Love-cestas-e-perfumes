...
const ShopProvider = ({ children }) => {
    ...
    // ATUALIZAÇÃO: Estado para endereços e frete automático
    ...
    const [autoShippingError, setAutoShippingError] = useState('');

    // Estado antigo de frete (para cálculo manual se necessário)
    ...
    // ATUALIZAÇÃO: Nova função para buscar endereços
    ...
    }, [isAuthenticated, isLoading, fetchPersistentCart, fetchAddresses]);
    
    // ATUALIZAÇÃO: Efeito para cálculo automático de frete
    useEffect(() => {
        if (cart.length > 0 && defaultAddress?.cep) {
            setIsLoadingAutoShipping(true);
            setAutoShippingError('');
            setAutoCalculatedShipping(null);
            ...
            const calculateAutoShipping = async () => {
                try {
                    ...
                    const options = await apiService('/shipping/calculate', 'POST', {
                        cep_destino: defaultAddress.cep,
                        products: productsPayload,
                    });
                    
                    // Filtra para encontrar apenas o PAC
                    const pacOption = options.find(opt => opt.name.toLowerCase().includes('pac'));
                    
                    if (pacOption) {
                        setAutoCalculatedShipping(pacOption);
                        // Define o PAC como a opção selecionada para o checkout
                        setSelectedShipping(pacOption); 
                    } else {
                        setAutoShippingError('Frete PAC não disponível para este CEP.');
                    }
                } catch (error) {
                    setAutoShippingError(error.message || 'Não foi possível calcular o frete.');
                } finally {
            ...
            // Limpa o frete se o carrinho estiver vazio ou não houver endereço padrão
            setAutoCalculatedShipping(null);
            setSelectedShipping(null);
        }
    }, [cart, defaultAddress]);
    ...
    return (
        <ShopContext.Provider value={{
            ...
            // Frete Manual (mantido para flexibilidade)
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                {cart.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 bg-gray-900 rounded-lg border border-gray-800 p-4 md:p-6 space-y-4">
                            {cart.map(item => (
                            ...
                            <div className="space-y-2 mb-4">
                                ...
                                <div className="flex justify-between text-gray-300">
                                    ...
                                    {isLoadingAutoShipping ? (
                                        ...
                                        <span className="text-xs text-gray-500">Adicione um endereço padrão</span>
                                    )}
                                </div>
                                {autoCalculatedShipping?.delivery_time && (
                                    <div className="text-right text-xs text-gray-400 -mt-1">
                                        Entrega em até {autoCalculatedShipping.delivery_time} dias úteis
                                    </div>
                                )}
                                {autoShippingError && <p className="text-red-400 text-sm text-right">{autoShippingError}</p>}
...
const CheckoutPage = ({ onNavigate }) => {
    ...
    const [paymentMethod, setPaymentMethod] = useState('mercadopago'); // Padrão
    const [isLoading, setIsLoading] = useState(false);
    const [isAddressModalOpen, setIsAddressModalOpen] = useState(false);

    // Seleciona o endereço padrão assim que ele for carregado
    useEffect(() => {
        if (defaultAddress) {
    ...
    const handleSaveAddress = async (formData) => {
        try {
            ...
            setSelectedAddress(savedAddress); // Seleciona o novo endereço
    ...
    const handlePlaceOrderAndPay = async () => {
        if (!selectedAddress || !paymentMethod || !selectedShipping) {
            notification.show("Por favor, selecione um endereço e forma de pagamento.", 'error');
            return;
        }
        setIsLoading(true);
        ...
        try {
            const orderPayload = {
                ...
                total: total,
                shippingAddress: selectedAddress, // Envia o objeto de endereço completo
            ...
            <AnimatePresence>
                {isAddressModalOpen && (
                    <Modal isOpen={isAddressModalOpen} onClose={() => setIsAddressModalOpen(false)} title="Adicionar Novo Endereço">
                        <AddressForm onSave={handleSaveAddress} onCancel={() => setIsAddressModalOpen(false)} />
                    </Modal>
                )}
            </AnimatePresence>
                        ...
                        <div className="lg:col-span-1 space-y-8">
                            <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-2xl font-bold text-amber-400">1. Endereço de Entrega</h2>
                                    <button onClick={() => onNavigate('account/addresses')} className="text-sm text-amber-400 hover:underline">Gerenciar</button>
                                </div>
                                <div className="space-y-3">
                                    {addresses.length > 0 ? (
                                        addresses.map(addr => (
                                            <div key={addr.id} onClick={() => setSelectedAddress(addr)}
                                                className={`p-4 border-2 rounded-lg cursor-pointer transition-all flex items-start gap-4 ${selectedAddress?.id === addr.id ? 'border-amber-400 bg-amber-900/50' : 'border-gray-700 hover:border-gray-600'}`}>
                                                <div className="flex-shrink-0 mt-1">
                                                    {selectedAddress?.id === addr.id ? <CheckCircleIcon className="h-6 w-6 text-amber-400"/> : <div className="w-6 h-6 rounded-full border-2 border-gray-600"></div>}
                                                </div>
                                                <div className="flex-grow">
                                                    <p className="font-bold">{addr.alias} {addr.is_default ? <span className="text-xs bg-amber-400 text-black px-2 py-0.5 rounded-full ml-2">Padrão</span> : ''}</p>
                                                    <p className="text-sm text-gray-300">{addr.logradouro}, {addr.numero}</p>
                                                    <p className="text-sm text-gray-400">{addr.bairro}, {addr.localidade} - {addr.uf}</p>
                                                    <p className="text-sm text-gray-400">{addr.cep}</p>
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                                        <p className="text-gray-400 text-center py-4">Nenhum endereço cadastrado.</p>
                                    )}
                                    <button onClick={() => setIsAddressModalOpen(true)} className="w-full mt-2 text-amber-400 border border-amber-400 rounded-md py-2 hover:bg-amber-400 hover:text-black transition">
                            ...
                            <div className="bg-gray-900 rounded-lg border border-gray-800 p-6 h-fit md:sticky md:top-28">
                                ...
                                <div className="border-t border-gray-700 mt-4 pt-4">
                                    ...
                                    {selectedShipping ? (
                                        ...
                                        <div className="text-gray-400 text-sm text-center py-1">Selecione um endereço para calcular o frete.</div>
                                    )}
...
const CrudForm = ({ item, onSave, onCancel, fieldsConfig, brands = [], categories = [] }) => {
    ...
    const handleSubmit = (e) => {
        ...
        // Apenas processa o campo 'images' se ele realmente existir na configuração do formulário.
